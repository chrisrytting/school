...
class myRSA(object):
    ...
    def generate_keys(self, p, q, e):
        ...
        n = p*q
        phi_n = (p-1)*(q-1)
        temp, dprime, xprime = egcd(e,phi_n)
        d = dprime % phi_n
        self.public_key = (e, n)
    ...
    def encrypt(self, message, key=None):
        ...
        e = key[0]
        n = key[1]
        m = rsa.partition(message, rsa.string_size(n), '~')
        c_list = []
        for message in m:
            tempmess = rsa.string_to_int(message)
            tempmess = long(tempmess)
            tempmess = pow(tempmess, e, n)
            c_list.append(tempmess)
        return c_list
    ...
    def decrypt(self, ciphertext):
        ...
        strings = []
        for mess in ciphertext:
            tempmess = pow(mess, d,n)
            tempmess = rsa.int_to_string(tempmess)
            tempmess = tempmess.rstrip('~')
            strings.append(tempmess)
...
def debug():
    ...
    print test_myRSA(message,287117,104729,610639)

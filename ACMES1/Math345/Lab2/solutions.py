...
def normal_var(n):
    ...
    final value.
    """
    randmat = np.random.normal(0,1,(n,n))
    means = np.mean(randmat, axis = 1)
    var = np.var(means)
    return var
    pass
...
for i in nvalues:
    ##########################print normal_var(i)
    print 'Variance gets less and less as n gets higher.'
    pass
...
def laplace(A, tolerance):
    ...
    while diff >= tolerance:
        i+=1
        print i
        Acopy[1:-1,1:-1] = (A[1:-1,:-2] + A[1:-1,2:] + A[2:,1:-1] + A[:-2,1:-1])/4
        diff = np.max(np.abs(Acopy -A))
        A[1:-1,1:-1] = Acopy[1:-1,1:-1]
    return A
    pass  
...
def blue_shift():
    ...
    """
    randints1 = np.random.randint(0,255,(100,100,3))
    toner = np.array([.5,.5,1])
    randints2 = randints1 *toner
    return randints1, np.round(randints2)
    pass
